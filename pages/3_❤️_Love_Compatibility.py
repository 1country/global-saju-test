...
def get_love_report(u_elem, p_elem, u_gender, p_gender, lang):
    ...
    # 🌟 [수정 핵심] 성별 호칭을 언어(lang)에 따라 다르게 정의
    if lang == "ko":
        if u_gender == "Male":
            me = "남자(본인)"
            partner = "여자(상대)"
    ...
    # 🌟 시나리오 데이터 (f-string으로 me, partner 변수 사용)
    reports = {
        "Same": {
            "score": 85,
            "ko": {
                "title": "🤝 거울 속의 연인: 운명적 동질감과 자존심의 대결",
                "chemistry": f"""두 사람은 처음 만나는 순간부터 **'이 사람, 나랑 진짜 비슷하다'**는 느낌을 강하게 받았을 것입니다. 마치 잃어버린 반쪽을 찾은 것처럼 대화 코드, 웃음 포인트, 심지어 싫어하는 것까지 똑같습니다. 말하지 않아도 서로의 기분을 알아채는 **텔레파시 커플**입니다.
...
서로가 서로에게 가장 친한 친구이자, 가장 뜨거운 연인이 될 수 있는 완벽한 파트너입니다. 함께 있으면 세상 무서울 것이 없는 든든한 동지가 되며, 데이트를 할 때도 친구처럼 편안하고 즐거운 분위기가 계속됩니다. 지루할 틈이 없는 유쾌한 관계입니다.""",
                "conflict": f"""하지만 **'너무 똑같다'**는 것이 치명적인 단점이 되기도 합니다. 두 사람 모두 자아와 고집이 강해서(비견), 한 번 싸움이 붙으면 절대 물러서지 않습니다.
...
3. **친구 같은 데이트:** 로맨틱한 분위기보다 활동적인 데이트가 사랑을 키웁니다."""
            ...
            "en": {
                ...
                "chemistry": f"You felt an instant connection, as if looking into a mirror. You share the same humor, values, and dislikes. A telepathic connection exists between you two. You are best friends and lovers.",
                "conflict": f"Being too similar is the trap. Both have strong egos and refuse to back down in arguments. You might hate seeing your own flaws reflected in your partner.",
        ...
        "Output": {
            ...
            "ko": {
                ...
                "chemistry": f"""**{me}**이 **{partner}**를 자식처럼 예뻐하고 챙겨주는 관계입니다. 본인은 상대방을 보기만 해도 귀여워서 어쩔 줄 모르고, 맛있는 것이 있으면 하나라도 더 먹이고 싶어 합니다. 상대방 역시 당신의 무한한 사랑 속에서 안정감을 느끼고, 당신을 전적으로 의지하게 됩니다.
...
세상에서 가장 **이타적이고 희생적인 사랑**을 하는 커플입니다. 주는 사람은 주는 기쁨을, 받는 사람은 받는 행복을 누리니 이보다 더 평화로울 수 없습니다. 주변 사람들이 보기에도 "정말 잉꼬부부다"라고 부러워할 만한 그림입니다.""",
                "conflict": f"""문제는 **{me}**이 지칠 때 발생합니다. "나는 이만큼 해줬는데, 너는 왜 나한테 그만큼 안 해줘?"라는 보상 심리가 생기는 순간 서운함이 폭발합니다. 또한, **{me}**의 관심이 지나치면 **{partner}**에게는 간섭과 잔소리(통제)로 느껴질 수 있습니다. 엄마와 사춘기 자녀처럼 투닥거릴 수 있는 위험이 있습니다.""",
                "intimacy": f"""침대에서도 **{me}**이 분위기를 리드하고 봉사하는 형태입니다. 상대방의 만족을 위해 최선을 다하며, 거기서 기쁨을 느낍니다. 감정적인 교감이 매우 풍부하고 로맨틱한 관계입니다.""",
                "future": f"""결혼 인연으로 아주 강력합니다. 특히 자녀가 생기면 관계가 더욱 단단해집니다. **{me}**이 집안의 대소사를 주도하고, **{partner}**는 잘 따르는 안정적인 가정이 됩니다. 다만 **{me}**이 혼자 모든 짐을 짊어지지 않도록 역할 분담이 필요합니다.""",
...
3. **표현 요구하기:** 상대방에게 "고맙다"는 말을 자주 해달라고 요청하세요."""
        ...
        "Wealth": {
            ...
            "ko": {
                ...
                "chemistry": f"""두 사람은 서로에게 **강렬한 성적 매력**을 느낍니다. 특히 **{me}**에게 **{partner}**는 "내 것으로 만들고 싶다"는 정복욕을 자극하는 대상입니다. 첫눈에 반했거나, 만나는 순간부터 스파크가 튀었을 확률이 높습니다.
...
남자가 여자를 만난 경우라면(남자의 재성=여자), 남자가 여자를 리드하고 여자가 잘 따르는 **가장 이상적인 궁합** 중 하나입니다. 반대로 여자가 남자를 만난 경우라면, 여자가 남자를 쥐락펴락하는 '여왕님과 머슴' 같은 재미있는 관계가 됩니다.""",
                "conflict": f"""이 관계의 핵심은 **'통제'**입니다. **{me}**이 상대를 내 뜻대로 조종하려 들면 상대방은 숨이 막혀 도망치고 싶어집니다. 집착과 의심이 싹트기 쉬운 관계이기도 합니다. 또한, 현실적인 문제(돈, 직업)로 인해 계산적인 관계가 될 수도 있으니 순수한 마음을 잃지 않도록 주의해야 합니다.""",
...
3. **존중하기:** "내 말대로 해"라는 명령조의 말투를 버리세요."""
        ...
        "Power": {
            ...
            "ko": {
                ...
                "chemistry": f"""**{partner}**가 **{me}**을 리드하고 통제하는 관계입니다. **{me}**은 상대방에게서 묘한 카리스마와 어른스러움을 느끼고 존경심을 갖게 됩니다.
...
여자가 남자를 만난 경우라면(여자의 관성=남자), 여자가 남자에게 보호받고 사랑받는 **전통적인 길연(吉緣)**입니다. 남자가 울타리가 되어주니 안정감이 듭니다. 반대로 남자가 여자를 만난 경우라면, 여자의 기가 세서 남자가 꼼짝 못 하는 '공처가' 커플이 될 수 있지만, 여자의 말만 잘 들으면 자다가도 떡이 생기는 궁합입니다.""",
                "conflict": f"""**{me}**이 느끼기에 **{partner}**는 너무 깐깐하거나 보수적일 수 있습니다. 상대방의 조언이 **'지적질'**이나 **'잔소리'**로 들리기 시작하면 스트레스가 극에 달합니다. "너는 왜 맨날 나를 가르치려 들어?"라는 불만이 터져 나올 수 있습니다.""",
...
3. **규칙 정하기:** 서로 간섭하지 말아야 할 선을 정하세요."""
        ...
        "Resource": {
            ...
            "ko": {
                ...
                "chemistry": f"""**{partner}**가 **{me}**을 헌신적으로 뒷바라지해주는 관계입니다. **{me}**은 가만히 있어도 상대방이 알아서 챙겨주고, 이해해주고, 용서해줍니다. 마치 엄마 품에 있는 것처럼 세상에서 가장 편안한 안식처를 만난 셈입니다.

정서적인 결속력이 매우 강해서, 말하지 않아도 서로의 아픔을 치유해주는 **'힐링 커플'**입니다. 밖에서 힘들었던 일도 이 사람만 만나면 눈 녹듯 사라지는 마법 같은 관계입니다.""",
                "conflict": f"""너무 편안하다 보니 **권태기**가 빨리 올 수 있습니다. **{me}**이 게을러지거나 상대방을 당연하게 여기는 순간 위기가 옵니다. 또한, 상대방의 사랑이 과해지면 **'집착'**이나 **'과잉보호'**로 느껴져 답답해질 수 있습니다. "나를 어린애 취급 하지 마"라고 반항할 수 있습니다.""",
